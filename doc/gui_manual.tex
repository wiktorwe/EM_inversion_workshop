...
\begin{itemize}
  ...
  \item RockEM suite binaries: \texttt{\textasciitilde/software/rockem-suite/bin/mpiEmmodADITE2d} and \texttt{\textasciitilde/software/rockem-suite/bin/mpiEminvADITE2d}
\end{itemize}
...
Command:
\begin{quote}
  \texttt{pip install voila ipywidgets plotly numpy ipykernel matplotlib scipy segyio}
\end{quote}
...
The MPI inversion binary (e.g.\ \texttt{\textasciitilde/software/rockem-suite/bin/mpiEminvADITE2d}) must be installed and the path in \texttt{runinv.sh} must be correct. The GUI and run script reference this binary.
...
\begin{itemize}
  ...
  \item \textbf{Missing MPI FD binary} — 02\_visualization and 04\_results (when generating synthetics) need the MPI FD binary (e.g.\ \texttt{\textasciitilde/software/rockem-suite/bin/mpiEmmodADITE2d}). Check the path in \texttt{FDmodel/runmod.sh} and that the binary exists.
  \item \textbf{Missing MPI inversion binary} — 03\_inversion needs the MPI inversion binary (e.g.\ \texttt{\textasciitilde/software/rockem-suite/bin/mpiEminvADITE2d}). Check the path in \texttt{scripts/templates/runinv.sh} and that the binary exists.
